...
ダウンロードのフェーズ
'''
# f = urllib2.urlopen('https://www3.nhk.or.jp/n-data/opendata/coronavirus/nhk_news_covid19_prefectures_daily_data.csv')
# print f.read() # 開いたファイルの中身を表示する
# f.close()

'''
加工のフェーズ
NHKのcsvファイルをcorona.datみたいな形状にする
...
class DataProcessing:
	...
	def txt_output(self):
		with open('./coronaData.txt', 'w') as f:
			...
			f.write(number + '\n')
			for i, pref in enumerate(self.prefecture):
				line = '#' + str(i) +': ' + str(self.pop[i]) + ' ' + str(self.infected_perday_dict[pref]) + ' ' + '#-' + str(i+1) + pref
				f.write(line + '\n')
